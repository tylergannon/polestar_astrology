...
class Stem < ActiveRecord::Base
  ...
  def self.find_by_ordinal(i)
    Rails.cache.fetch "stem-#{i}" do
      find_by ordinal: i
    end
  end

  STEMS = %i(nothing jia yi bing ding wu ji geng xin ren kui)

  def self.find_by_pinyin(pinyin)
    find_by_ordinal STEMS.index(pinyin.to_sym)
  end

  def next
    self.find_by_ordinal(ordinal % 10 + 1)
  end
  ...
  def Stem.reseed
    ...
    %w(癸 guǐ mizunoto ki 계  gye quý 陰 yin 水  water  北 North gui 10)].each_with_index do |arr, i|
      stem = Stem.create :char => arr[0], :pinyin => arr[1], :yin_yang => arr[8], :element => arr[10],
        :normalized => arr[13], :ordinal => arr[14].to_i, id: arr[14].to_i
    end
  end
